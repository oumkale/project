    ...
    def post(self, request, *args, **kwargs):
        if not request.user.applicant.taken_personality_test and not request.user.is_staff:
            ...
            if test_type == '1':
                ...
                print("F3 : ")
                print(f3)
                print(f3str)    #pickle_in = open(os.path.join(classifer_base, 'conscientious.pkl'), 'rb')
                ...
                #f2 = sum(choices)
                f4 = 38 - choices[0]+choices[1]-choices[2]+choices[3]-choices[4]-choices[5]-choices[6]-choices[7]-choices[8]-choices[9]
        ...
        if request.user.applicant.taken_personality_test and not request.user.is_staff:
            ...
            if len(averages) != 5 or None in averages:
                ...
                if result in possible_results:
                    ...
                    user = User.objects.get(username=request.user.username)
                    user.applicant.is_employable = "ABC"
                    user.applicant.save()
                else:
                    completed = False
        else:
